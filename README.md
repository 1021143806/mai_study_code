...
| 维度 | 麦麦学代码 | DSH（compaction-basic） |
|---|---|---|
| **触发时机** | 主动/定时：TTL 30 分钟、LRU 满、话题切换、窗口超限 | **按上下文压力自动触发**（`thresholdRatio: 0.8`，用到 80% 时才压缩） |
| **保留策略** | 按 LRU/最旧丢弃（简单粗暴腾空间） | **`retainRatio: 0.08`**（智能保留最新 8% 高价值上下文） |
| **LLM 是否介入** | **不调 LLM**，直接丢弃旧轮次 | **调用 LLM 智能压缩**（`compactionRetries: 1`），先压缩旧内容再丢弃细节 |
| **Token 上限** | 固定缓存容量 | `maxTokens: 8192` 预算控制 |
| **对记忆的影响** | 丢弃 = 丢失旧上下文 | 压缩 = 保留摘要语义，仅丢细节 |

> **结论**：DSH 的清理机制更优。`mai_study_code` 是"被动腾空间"（到阈值就丢最旧的），而 DSH 是"智能压缩"（先让 LLM 把旧上下文压成摘要、保留语义，再丢弃冗余细节）。前者省 token 但丢记忆，后者既省 token 又保住关键语义——这正是 DSH 在"记忆闭环"上更成熟的一个体现。
